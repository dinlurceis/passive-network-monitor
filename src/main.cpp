...
int main() {
    ...
    try {
        ...
        // REST API Server
        g_api_server = std::make_unique<pnads::RestServer>(cfg.db_connection_string(), cfg.api_port, &pcap_queue);
        g_api_server->start();
        ...
        std::atomic<uint64_t> pkt_arp{0}, pkt_dhcp{0}, pkt_mdns{0},
                               pkt_ssdp{0}, pkt_dns{0};
        auto callback = [&](const pnads::RawPacket& pkt) {
            ...
            if (eth->ethertype == pnads::ETHERTYPE_ARP) {
                ...
                if (arp) {
                    pkt_arp++;
                    spdlog::debug("ARP op={} {}->{}", arp->opcode, arp->sender_ip, arp->target_ip);
        ...
        if (live) {
            ...
            std::string bpf_filter = "arp or (udp and (port 67 or port 68 or port 5353 or port 1900 or port 53))";
            if (!g_reader->set_filter(bpf_filter)) g_reader->set_filter("arp or (udp and (port 67 or port 68))");
            ...
            if (stable_pcap.empty()) {
                ...
                if (std::filesystem::exists(pcaplist_dir)) {
                    ...
                    for (const auto& entry : std::filesystem::directory_iterator(pcaplist_dir)) {
                    ...
                    if (!found.empty()) stable_pcap = found.front();
                }
            }

            if (stable_pcap.empty()) {
                spdlog::warn("No stable PCAP found. Set PCAP_FILE or place .pcap in /samples/pcaplist/");
                spdlog::info("Idle mode: API listening on :{} — waiting for uploads", cfg.api_port);
                // Chỉ chờ upload
                while (g_running) {
                    ...
                    if (!next_file.empty()) {
                        try {
                            ...
                            g_reader->set_filter("arp or (udp and (port 67 or port 68 or port 5353 or port 1900 or port 53))");
                            g_reader->start(callback);
                        ...
                        { std::lock_guard<std::mutex> lock(pcap_queue.mutex); pcap_queue.current_pcap = ""; }
                    }
                }
            } else {
                spdlog::info("Stable PCAP (loop): {}", stable_pcap);
                spdlog::info("API listening on :{} — upload a PCAP to interrupt the loop", cfg.api_port);

                uint64_t loop_count = 0;

                // BPF filter dùng chung
                const std::string bpf_filter =
                    "arp or (udp and (port 67 or port 68 or port 5353 or port 1900 or port 53))";
                ...
                while (g_running) {
                    ...
                    if (!priority_file.empty()) {
                        ...
                        spdlog::info("[PRIORITY] Pausing stable loop — processing upload: {}",
                            priority_file.substr(priority_file.find_last_of("/\\") + 1));
                        uint64_t pkts_before = packet_count.load();
                        auto t0 = std::chrono::steady_clock::now();
                        try {
                            g_reader = std::make_unique<pnads::PcapReader>(priority_file, false);
                            ...
                            spdlog::info("[PRIORITY] Done: {} | {:.2f}s | {} pkts | {:.0f} pkt/s",
                                priority_file.substr(priority_file.find_last_of("/\\") + 1),
                                elapsed, processed, pps);
                            spdlog::info("[PRIORITY] Resuming stable loop...");
                        } catch (const std::exception& e) {
                            spdlog::error("[PRIORITY] Failed {}: {}", priority_file, e.what());
                        }
                        {
                    ...
                    if (loop_count % 10 == 1) {
                        spdlog::info("[STABLE] Loop #{} — {}", loop_count,
                    ...
                    try {
                        ...
                        spdlog::error("[STABLE] Failed to read {}: {}", stable_pcap, e.what());
                    ...
                    uint64_t processed = packet_count.load() - pkts_before;
                    double pps = (elapsed > 0) ? processed / elapsed : 0;
                    spdlog::info("[STABLE] Loop #{} done | {:.2f}s | {} pkts | {:.0f} pkt/s",
